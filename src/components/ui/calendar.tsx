  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        dropdown: cn(
          "absolute bg-popover inset-0 opacity-0 max-h-[500px] overflow-y-auto",
...
function CustomDropdown({ value, onChange, options, ...props }: DropdownProps) {
  ...
  return (
    <Select value={value?.toString()} onValueChange={handleChange}>
      <SelectTrigger className="pr-1.5 focus:ring-0 shadow-none border-none py-0 h-7 w-fit hover:bg-muted/50">
        <SelectValue>{selected?.label}</SelectValue>
      </SelectTrigger>
      <SelectContent position="popper" className="z-50">
        <div className="max-h-[300px] overflow-y-auto" data-lenis-prevent>
          {options?.map((option) => (
            <SelectItem
              key={option.value}
